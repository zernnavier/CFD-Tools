...
async def get_justify_size(read_file: Path) -> NodeElem:
    node_count, elem_count = 0, 0
    node, elem = "", ""
    async with aiofiles.open(read_file, mode="r") as i_file:
        async for line in i_file:
            print(line)
...
async def print_download_bar_percentage(percentage: int):
...
async def main():
    ...
    async with aiofiles.open(files.in_file, mode="r") as i_file, aiofiles.open(
        files.out_file, mode="w"
    ) as o_file:
        maximum, j_length = await get_justify_size(files.in_file)
        max_made, _, total = maximum
        max_elem = total - max_made
        node_just, elem_just = j_length
        await o_file.write(f"{max_made}    {max_elem}" + "\n")
        ...
        async for line in i_file:
            ...
            if reg_res_node or reg_res_elem:
                await o_file.write(processed_line + "\n")
                curr_percentage = int((node_count + elem_count) / total * 100)
            if (curr_percentage - old_percentage) == 1:
                asyncio.create_task(print_download_bar_percentage(curr_percentage))
